...
CREATE TABLE IF NOT EXISTS `dqm_security_info_snapshot` (
    ...
    `stkname` VARCHAR(100) NOT NULL DEFAULT '',
    `std_stkcode` VARCHAR(30) NOT NULL DEFAULT '',
    `mst_type` VARCHAR(50) NOT NULL DEFAULT '',
    `compn_stock_code` VARCHAR(20) NOT NULL DEFAULT '',
    `compn_stock_name` VARCHAR(100) NOT NULL DEFAULT '',
    `index_name` VARCHAR(100) NOT NULL DEFAULT '',
    `send_date` VARCHAR(20) NOT NULL DEFAULT '',
    `created_at` DATETIME DEFAULT CURRENT_TIMESTAMP,
    UNIQUE KEY `uk_snapshot_record` (`check_date`, `stkcode`, `mst_type`, `compn_stock_code`),
